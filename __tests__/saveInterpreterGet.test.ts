import { GET } from '@/app/api/saveInterpreter/route';
...
describe('/api/saveInterpreter GET', () => {
    it('should return match found if data exists', async () => {
        const mockRequest = createMockRequest('http://localhost/api/saveInterpreter?q=ABC');
    ...
    it('should return invalid query if no query param is provided', async () => {
        const mockRequest = createMockRequest('http://localhost/api/saveInterpreter');
        const response = await GET(mockRequest);
        const json = await response.json();
