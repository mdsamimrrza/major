import { DELETE } from '@/app/api/deleteComp/route';
...
it('should return deleted if record exists', async () => {
    ...
    const request = createMockRequest('http://localhost/api/deleteComp?q=123');
...
it('should return not deleted if no record exists', async () => {
    ...
    const request = createMockRequest('http://localhost/api/deleteComp?q=999');
    const response = await DELETE(request);
    const json = await response.json();
...
it('should return invalid query parameter when query param is missing', async () => {
    const request = createMockRequest('http://localhost/api/deleteComp');
    const response = await DELETE(request);
    const json = await response.json();
