import { GET } from '@/app/api/getComp/route';
...
it('should return match found if data exists', async () => {
    ...
    const request = createMockRequest('http://localhost/api/getComp?q=test@example.com');
...
it('should return match found with empty array if no data exists', async () => {
    ...
    const request = createMockRequest('http://localhost/api/getComp?q=notfound@example.com');
    const response = await GET(request);
    const json = await response.json();
...
it('should return invalid query when query param is missing', async () => {
    const request = createMockRequest('http://localhost/api/getComp');
    const response = await GET(request);
    const json = await response.json();
